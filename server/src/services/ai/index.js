...
import { renderContext, renderFacts, retrieveContext } from './context.js';
import { ACTION_JSON_SCHEMA, inspectAction } from './actions.js';
import { BASE_SYSTEM, MODE_HINTS, STRUCTURED_INSTRUCTIONS } from './prompts.js';
...
function buildSystem({ mode, today, blocks, denied, preferences }) {
  const header = [
...
export async function chat(userId, { conversationId, message, mode: explicitMode, context: anchorInput, date }) {
  ...
  const history = conversation
    ? await AIMessage.find({ conversation: conversation._id, user: userId, role: { $in: ['user', 'assistant'] } }).sort({ createdAt: -1 }).limit(MAX_HISTORY).lean()
    : [];
  const messages = [...history.reverse().map((m) => ({ role: m.role, content: m.content || '…' })), { role: 'user', content: message }];

  const { blocks, used, denied, explicit, anchorScope, anchorUsed } = await retrieveContext(userId, { message, mode, anchor, preferences, today });
  ...
  if (onlyDenied || (anchorBlocked && !used.length)) {
    ...
    const system = buildSystem({ mode, today, blocks, denied, preferences });
    const result = await callProvider(() =>
      provider.structured({ system, messages, schema: ACTION_JSON_SCHEMA, effort: mode === 'ask' ? 'low' : 'medium', validate: isStructuredReply }),
    );
    ...
    ({ accepted, rejected } = await vetProposals(userId, result.data.actions, allowedScopes(preferences)));
    usage = result.usage;
  }
